...
class Select2Test extends KernelTestBase {
  ...
  public function testSelect2Theming() {
    ...
    $this->assertEquals(1, count($select2_js));
    $select2_css = $this->xpath("//link[contains(@href, 'select2/css/select2.seven.css')]");
    $this->assertEquals(1, count($select2_css));
    $select2_css = $this->xpath("//link[contains(@href, 'select2/css/select2.seven-overrides.css')]");
    $this->assertEquals(1, count($select2_css));
  }
